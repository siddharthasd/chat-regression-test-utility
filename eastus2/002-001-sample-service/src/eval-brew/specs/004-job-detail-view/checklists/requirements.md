...
- **Re-validated 2026-05-29 (Round 5 — user-feedback removed)** after the cross-spec consistency pass: User Story 3 dropped, `FR-007`'s User Feedback column dropped, `FR-010` removed, `SC-005` removed, `FR-008` / `FR-016` / `SC-002` / Job Detail Projection / Row Detail Projection / Assumptions all trimmed of feedback references. The two earlier feedback-specific clarification Q&As (interaction model, read-write) are now marked superseded. The earlier Round-2 note above mentioning `FR-010` and feedback interaction history is preserved as historical context — the FR itself is now a "removed in v1" placeholder. All 16 checklist items remain passing.
