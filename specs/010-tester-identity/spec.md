...
**Input**: User description: "Module 3 — Authentication & User Identity. Two distinct layers: (1) **Tester Identity** — the real human at the harness, OS-derived (no login, no auth flow), used for attribution / logging / UI display; and (2) **Test Credentials** — per-utterance `testId` + `password` from the CSV, opaque to the harness, passed to the connector for chatbot authentication. Tester identity is resolved at startup via `os.getlogin → getpass.getuser → 'unknown-user'`, stored in the application context read-only, auto-stamped on `Job.createdBy` and `TesterFeedback.feedbackBy`, displayed in a 'Logged in as' UI indicator, written to log line prefixes, and surfaced in the CLI `harness info` output. Test credentials are masked everywhere they surface (`••••••••` in UI, `[MASKED]` in exports, redacted in logs)."
...
- Q: Tester identity attribution — re-introduce `Job.createdBy` / `TesterFeedback.feedbackBy` populated from the OS user, reversing the no-creator precedent set across 7 prior specs? → A: **Yes.** Re-introduce both fields as OS-derived strings, auto-populated at write time, immutable, not tester-editable. Add the "Logged in as: <user>" indicator to every UI surface. New parent `FR-026` formalizes the resolution rule; lock-step amendments land on `001`/`002`/`003`/`004`/`005`/`009`. There is still no login flow, no credentials challenge, no override.
...
When the harness process launches, before any module reads or writes persistent state, the framework resolves the OS-derived tester identity using a documented fallback chain. The resolved value is held in a read-only application context for the lifetime of the process. Every subsequent persistence write that needs an "operator" stamp (Job creation, feedback application) pulls the identity from this context — no module ever re-resolves it, no tester ever supplies their own value, and the identity cannot change mid-process.

**Why this priority**: Without this, the attribution story falls apart — every other module that needs `createdBy` / `feedbackBy` would have to fend for itself, leading to drift. This is the MVP slice.
...
### User Story 2 - Persisted attribution on Jobs and Feedback (Priority: P1)

Every Job's `createdBy` field carries the OS-derived tester identity captured at job-creation time. Every TesterFeedback row's `feedbackBy` field carries the OS-derived tester identity captured at the moment that specific feedback was applied. The tester does not supply, see, or edit these values via any form control. The values are surfaced in the dashboard's Created By column, the detail view's Metadata Panel, the export's job metadata block, and (optionally) the per-row export block for feedback. This is the spec's "attribution" promise made operational across all surfaces.
...
**Independent Test**: Boot harness as OS user `"alice"`, create a Job via the wizard, verify `Job.createdBy == "alice"` in the database. Switch to user `"bob"` (e.g., second test in CI), open the same Job's detail view, apply thumbs-down feedback on one row, verify the resulting `TesterFeedback.feedbackBy == "bob"` (the act-time identity, not the job-creation identity).
...
2. **Given** a Job exists with `createdBy == "alice"`, **When** the harness is later restarted by OS user `"bob"` and `"bob"` opens that job's detail view and applies feedback on a row, **Then** the resulting TesterFeedback row carries `feedbackBy == "bob"` (NOT `"alice"`). The Job's `createdBy` remains `"alice"`.
3. **Given** any Job exists, **When** the dashboard renders, **Then** the Created By column shows the persisted `Job.createdBy` value for that row.
4. **Given** any Job exists, **When** the detail view's Metadata Panel renders, **Then** the Created By field shows the persisted `Job.createdBy` value.
5. **Given** any Job is exported, **When** the export's job-metadata block is inspected, **Then** the `createdBy` field is present with the persisted value (per `005 FR-005`).
...
**Independent Test**: Boot the harness as `"alice"`, drive a few operations (create job, apply feedback, run a job), inspect the log file, verify every line includes `"alice"` as a prefix or structured field.
...
For every CSV row, the `testId` and `password` are passed from the wizard's Step 2 upload through the orchestrator to the connector's `sendUtterance(handle, utterance, testId, password)` call. The harness does NOT validate them, does NOT store the `password`, does NOT log them. `testId` is persisted (per `009 FR-002`), surfaced in the contract (per `006 FR-002`), the ExecutionResult (per `009 FR-003`), the detail view's results table (per `004 FR-007`), and the export (per `005 FR-006`). `password` appears in NONE of those — it's a per-row value used once at the moment of connector invocation and dropped from memory afterward. This story consolidates rules already enforced by 6 other modules; nothing new is added here.
...
**Independent Test**: Upload a CSV containing rows with distinct `(testId, password)` pairs to a job. Run the job to completion. Inspect: (a) the database file's bytes — no password value should appear in plaintext or encrypted form (verifying `009 SC-004`); (b) the detail view's results table — `testId` column shows the values; password is nowhere visible; (c) the export — `testId` column present, password column shows `[MASKED]` or is absent; (d) the log file — no password value appears anywhere.

**Acceptance Scenarios**:

1. **Given** a CSV row carries a `(testId, password)` pair, **When** the orchestrator processes the row, **Then** `sendUtterance` is invoked with both values as parameters (per `007 FR-003`); the password is NOT logged before, during, or after the call.
...
3. **Given** the row is rendered in the detail view, **When** the page displays, **Then** `testId` is shown in full; the `password` is shown as the masked placeholder `••••••••` or as the literal `[MASKED]` token — never the underlying value (per `004 FR-005`).
4. **Given** the row appears in any export, **When** the export is generated, **Then** the password column shows `[MASKED]` or is absent; the actual password is not in any output byte (per `005 FR-006`, `005 FR-011`).
...
- A tester applies feedback, then re-applies (replacing) feedback in a later session as a different OS user — the new TesterFeedback row's `feedbackBy` reflects the second session's user; the prior row was deleted (per `009 FR-004` replace-via-delete semantics) so no history of the original `feedbackBy` is retained.
...
- **FR-004**: The tester identity MUST be auto-stamped onto persisted attribution fields at write time:
  ...
  - `TesterFeedback.feedbackBy` at feedback-write time (per `009 FR-004`, detail view `004 FR-010`).
...
- **FR-009**: The CSV's required schema MUST include at minimum the columns `utteranceText`, `testId`, and `password`, all non-empty per row. CSV-level validation (column presence, non-empty values, encoding correctness) is performed by Module 8 (CSV Upload & Validation Service, TBD). This module does NOT implement the CSV parsing or validation; it only documents the credential-handling rules other modules must follow.
...
- **FR-011**: The original uploaded CSV file MUST NOT be retained on disk as a binary artifact. The Module 3 input's directive to store the raw CSV with OS-level filesystem permissions was explicitly NOT applied. Only the per-row persisted Utterance records (minus `password`) survive past upload. The detail view's source-CSV download (`004 FR-006`) reconstructs from those records.
- **FR-012**: The `password` MUST be passed to the connector via the `sendUtterance(handle, utterance, testId, password)` invocation defined by Module 4 (`007 FR-003`). The connector is responsible for all chatbot-side authentication logic — token acquisition, session management, retries against the chatbot's identity system. The harness core's only responsibility is delivering the raw values.
- **FR-013**: The per-row `password` MUST be masked everywhere it surfaces to a tester or to a non-harness consumer:
  - **In the UI** (detail view's results table, traceability expand): rendered as `••••••••` placeholder, never the underlying value (per `004 FR-005`'s secret-masking pattern).
  - **In exports**: rendered as `[MASKED]` literal in CSV / JSON, never the underlying value (per `005 FR-011` for connector secrets; this spec extends the same rule to CSV-row passwords).
...
- **FR-014**: The `testId` MUST be carried through the full per-row traceability chain in plaintext form: persisted on Utterance (`009 FR-002`), included in the Standard Evaluation Contract (`006 FR-002`), denormalized onto ExecutionResult (`009 FR-003`), shown as a filterable column in the detail view's results table (`004 FR-007`, `004 FR-012`), and included in exports (`005 FR-006`). The `testId` is NEVER masked.
- **FR-015**: For the same utterance text appearing with different `(testId, password)` pairs across rows, the harness MUST treat each as an independent execution. Connectors MAY share session state internally across rows with the same `testId`, but the harness's per-row records remain independent.
- **FR-016**: Connector implementations MUST NOT echo or leak the per-row `password` in their `RawResponse` payload (`007 FR-003`). MockConnector's behavior (`007 FR-022`) MUST echo only `testId` back, never `password`. This is a connector-implementation rule; the harness's enforcement is via the masking rules of `FR-013`.

### Key Entities *(include if feature involves data)*

- **Tester Identity (application-context value)**: A single read-only string resolved at process startup per `FR-001`. Held in the application context for the lifetime of the process. Auto-stamped onto `Job.createdBy` and `TesterFeedback.feedbackBy` per `FR-004`. Surfaced in UI per `FR-005`, in logs per `FR-006`, and in CLI per `FR-007`. Not a persisted entity — the persisted form is the field values on Job and TesterFeedback.
- **Test Credential (per-row, transient)**: A `(testId, password)` pair from one CSV row, held in memory only for the duration of that row's connector invocation. Carried as parameters to `sendUtterance` per `FR-012`. Not persisted in any form (per `FR-010`); never logged in plaintext (per `FR-013`).
...
- **SC-003**: Booting the harness as OS user `<X>` and applying feedback on any row results in the new TesterFeedback row's `feedbackBy == <X>` — verifiable by SQL inspection.
...
- The `Job.createdBy` and `TesterFeedback.feedbackBy` fields are NOT a substitute for authentication. They are attribution-only — useful for "who ran this on which laptop?" forensic questions, not for access control. There is no access control in the harness (it's a single-user local tool).
...
- "Modules 5/6" in the user's original Module 3 input refer to specific connector implementations (e.g., a Copilot Studio connector at our `011-` and a Semantic Kernel connector at our `012-`); none of those exist yet. This spec only requires that connector implementations honor the `sendUtterance(handle, utterance, testId, password)` interface defined in Module 4 (`007 FR-003`) and never leak the `password`.
