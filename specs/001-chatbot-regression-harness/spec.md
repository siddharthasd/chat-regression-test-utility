...
- **FR-015**: System MUST allow the tester to download a complete results export of any job as a single zip archive containing both: (a) a CSV with one row per test case row and nested structures JSON-stringified per cell, and (b) a JSON file with the same rows in nested, lossless form. Both files MUST include every row's input (excluding `password`), normalized output, evaluation result, error (if any), and feedback (if any).
...
- **Evaluation Result**: Per-row evaluation output. Carries: verdict, evaluator-specific structured fields, the agent's raw payload, timestamps, and error (if any).
