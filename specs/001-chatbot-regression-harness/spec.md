...
- **FR-002**: System MUST accept a CSV upload whose schema requires per row at minimum: `utteranceText`, `testId`, `password`. Additional columns MUST be preserved verbatim as per-row metadata.
...
- **Tester Feedback**: Optional per-row human verdict. Carries: thumbs-up or thumbs-down, `feedbackBy` (OS-derived tester identity at apply time, per `FR-026`), optional note, timestamp.
