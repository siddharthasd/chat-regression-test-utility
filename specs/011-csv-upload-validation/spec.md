...
**Why this priority**: Validation is what makes the upload step a contract. Without it, malformed CSVs would silently propagate corrupt rows into Utterance/ExecutionResult later. Equal-priority with US1 because both are required for the upload step to be trustworthy.

**Independent Test**: Submit three CSVs in sequence: (a) wrong-encoding CSV (e.g., UTF-16 BOM where UTF-8 is required), (b) CSV missing the `password` column, (c) CSV where row 3 has an empty `testId`. For each, verify: (i) the response is a validation-error structure naming the specific issue (encoding name, missing column name, or row/column path), (ii) the Utterance table for that jobId remains empty, (iii) Job.totalUtteranceCount remains 0 / unmutated, (iv) Job.sourceCSVFilename remains null / unmutated.
...
2. **Given** a CSV whose header row is missing one or more of the required columns (`utteranceText`, `testId`, `password`), **When** Module 8 parses the header, **Then** the upload is rejected; the response names "missing required column(s)" with the specific column name(s); no rows are persisted.
...
2. **Given** a CSV where a data row's `utteranceText` value is a quoted multi-line string (RFC 4180-style escaping), **When** Module 8 parses, **Then** the resulting Utterance's `originalText` carries the full value with embedded newlines preserved.
...
- **FR-006**: The service MUST recognize the following required header columns: `utteranceText`, `testId`, `password`. Header-name comparison MUST be case-sensitive but MUST trim surrounding whitespace from each header before comparison (e.g., `" utteranceText"` matches `"utteranceText"`). Missing required columns MUST be rejected with an error naming each missing column. Duplicate header columns MUST be rejected.
...
- **FR-013**: For every valid data row, the service MUST create one Utterance entity in the data layer (`009 FR-002`) carrying:
  ...
  - `originalText` — the row's `utteranceText` value, byte-equal to the source.
...
- **FR-015**: The service MUST surface the parsed `password` values to the orchestrator (Module 10, TBD) via an **in-memory mechanism** — a process-scoped store keyed by `(jobId, utteranceId) → password` is the canonical pattern; the exact shape (function call, in-process service object, DI binding) is plan-level. The store MUST NOT be backed by disk. The store MUST be inaccessible to non-harness processes.

  **Lifecycle rules** (in order of expected effect on each entry):

  - **Per-row eviction (primary)** — The orchestrator (Module 10, TBD) MUST remove a row's `(jobId, utteranceId)` entry from the store **immediately after** its `sendUtterance` call for that row returns, regardless of whether the connector call succeeded or failed. The entry's password MUST NOT be retained for retry, comparison, or any subsequent use.
  ...
  After these rules, any given `(jobId, utteranceId)` entry's lifetime in memory is bounded by: from upload completion to the orchestrator's per-row `sendUtterance` return — and never longer.
...
- **CSV Parse Result** *(transient, in-memory)*: The intermediate state between "file read" and "Utterance rows committed". Carries: the validated header row, an ordered list of parsed data rows (each with `utteranceText` / `testId` / `password` / extra-metadata), and a warnings list. Lives only for the duration of one upload attempt; discarded after commit (the `password` values transition into the in-memory password store per `FR-015`).
- **In-Memory Password Store** *(process-scoped, plan-level shape)*: A process-resident keyed lookup that maps `(jobId, utteranceId) → password` for the lifetime of the harness process (or shorter, on Job completion). The orchestrator (Module 10, TBD) reads from this store at per-row execution time. NEVER backed by disk. NEVER visible to non-harness processes. The exact implementation (Python dict in a service object, async-safe shared state, etc.) is plan-level.
...
- **SC-008**: A CSV containing UTF-8 BOM, embedded newlines in quoted values, and non-ASCII Unicode in `utteranceText` is parsed losslessly — verifiable by submitting a fixture and confirming the persisted `originalText` round-trips byte-equal to the input.
...
- **SC-011**: A re-upload to a draft Job that already has Utterance rows atomically replaces the prior rows + password-store entries with the new upload's data — verifiable by uploading CSV `A.csv` to a draft Job (creating N_A rows + N_A store entries), then uploading `B.csv` to the same Job, then inspecting both the database and the in-memory store: exactly N_B rows + N_B entries from `B.csv`, with no traces of `A.csv` remaining anywhere.
- **SC-012**: For any row whose `sendUtterance` invocation has returned (success or failure), no trace of its password remains in the in-memory store — verifiable by instrumenting the store with a probe that asserts no entry exists for any `(jobId, utteranceId)` whose orchestrator invocation has returned.
- **SC-013**: For any Job in a terminal status, no trace of any of its rows' passwords remains in the in-memory store — verifiable by probing the store immediately after a Job-terminal transition for any of the seven terminal status pathways (completed, failed, cancelled).
...
- Module 10 (Job Execution Engine, TBD) is the in-memory password store's consumer. The exact handoff mechanism is plan-level — Module 8 just produces, Module 10 consumes.
