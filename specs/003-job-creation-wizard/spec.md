...
- **FR-004**: Step 1 MUST require a non-empty job name. Description is optional. On the first Next from Step 1 of a brand-new wizard session, the system MUST create a Job record in `draft` status with the entered name, optional description, and a `created-at` timestamp.
...
- **FR-021**: The wizard MUST NOT expose any "Created By" / creator / authentication concept. Per the parent spec, the harness is single-user; the Job record stores no creator field.
