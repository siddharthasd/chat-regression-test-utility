...
def _upload(client, job_id, csv: str):
    return client.post(
        f"/jobs/{job_id}/step2",
        files={"csv_file": ("rows.csv", io.BytesIO(csv.encode()), "text/csv")},
    )
...
def test_create_to_start_happy_path(client, enqueue_spy) -> None:
    ...
    assert _upload(client, job_id, _GOOD_CSV).status_code in (302, 303)
    assert client.post(f"/jobs/{job_id}/step3", data={"connector_id": conn_id}).status_code in (302, 303)
    assert client.post(
...
def test_archived_connector_requires_reselect(client) -> None:
    ...
    body = client.get(f"/jobs/{job_id}/step5").text
...
def test_empty_evaluator_registry_affordance(client) -> None:
    ...
    _upload(client, job_id, _GOOD_CSV)
    client.post(f"/jobs/{job_id}/step3", data={"connector_id": conn_id})
    body = client.get(f"/jobs/{job_id}/step4").text
