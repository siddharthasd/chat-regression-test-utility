...
async def run_turn(
    ...
    All DB writes happen via a fresh get_session() context so this coroutine
    is decoupled from the per-request session.
    """
    ...
    # Decrypt credentials from the session snapshot
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=None) as client:
            try:
                async with client.stream(
                    "POST", connector_url, json=connector_body
                ) as response:
        ...
        async with httpx.AsyncClient(timeout=None) as client:
            try:
                async with client.stream("POST", evaluator_url, json=contract) as response:
                    response.raise_for_status()
...
def _validate_contract(contract: dict) -> None:
    """Minimal structural check: require utterance and chatbotResponse keys."""
    if not isinstance(contract, dict):
        raise ValueError("Contract must be a JSON object")
    if "utterance" not in contract:
        raise ValueError("Contract missing required field 'utterance'")
    if "chatbotResponse" not in contract:
        raise ValueError("Contract missing required field 'chatbotResponse'")
